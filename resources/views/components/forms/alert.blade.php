...
@php
    // Define styles and icons for different types of alerts
    $alertStyles = [
        'error' => 'alert-error',
    ...
    $icons = [
        ...
        'success' => '<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 stroke-current" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>',
        'info' => '<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 stroke-current" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M12 18h.01" /></svg>',
    ];
@endphp

<div role="alert" class="alert {{ $alertStyles[$type] }}">
    {!! $icons[$type] !!}
    <span>{{ $slot }}</span>
</div>
